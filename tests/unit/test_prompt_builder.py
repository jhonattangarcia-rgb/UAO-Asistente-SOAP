"""Tests for services.prompt_builder: system and user prompt construction."""
...
def test_build_system_prompt_contains_soap_keywords() -> None:
    """build_system_prompt() must mention SOAP and the justification section marker."""
    result = build_system_prompt()
...
def test_build_user_prompt_returns_string() -> None:
    """build_user_prompt() must return a non-empty string for valid input."""
...
def test_build_user_prompt_interpolates_parameters() -> None:
    """build_user_prompt() must embed both input parameters verbatim in its output."""
...
def test_build_user_prompt_handles_empty_parameters() -> None:
    """build_user_prompt() must still return a non-empty string when inputs are empty."""
